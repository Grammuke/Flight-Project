...
const ProfileImageBox: FC<profileProps> = ({ size, round = true }) => {
  const profileStyle = ScaledSheet.create({
    imgSize: {
      width: size ? `${size}@s` : "70@s",
      height: size ? `${size}@s` : "70@s",
    },
  });
  const { profileData } = {
    profileData: {
  ...
  const uri = profileData?.avatar_link;
  return (
    <Box
      style={[
        profileStyle.imgSize,
        globalStyle.borderRadius8,
        round && globalStyle.br,
      ]}
    >
      <ImageComponent
        style={
